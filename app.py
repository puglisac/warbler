...
from forms import UserAddForm, LoginForm, MessageForm, EditUserForm, NewPasswordForm
...
def handle_like(msg_id):
    ...
    user = User.query.get_or_404(g.user.id)
    if liked in user.likes:
        """removes like from user if msg is already liked"""
        user.likes.remove(liked)
        db.session.commit()
        return redirect("/")
    else:
        """adds like to user"""
        user.likes.append(liked)
        db.session.commit()
        return redirect("/")
...
def show_likes(user_id):
    """shows posts that a user has liked"""
    user = User.query.get(user_id)
    ids = [msg.id for msg in user.likes]
    messages = Message.query.filter(Message.id.in_(
        ids)).order_by(Message.timestamp.desc())
...
def homepage():
    ...
    if g.user:
        ...
        messages = (Message
                    .query.filter(or_(Message.user == user, Message.user_id.in_(following)))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
